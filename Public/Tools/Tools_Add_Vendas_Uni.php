<?php
    if(!empty($_GET['pago']))
    {
        ...
        if($pago >= $total and $pago > 0 )
        {
            ...
            $sql_2 = "SELECT * FROM pagar_pedido";
            ...
            if($result_Select_2->num_rows > 0)
            {
                ...
                while($user_data = mysqli_fetch_assoc($result_Select_2))
                {
                    ...
                    $id_venda_uni = $contagem;

                    $sql_Insert = "INSERT INTO admin_vendas_uni (id, id_venda_uni, dia, hora, id_fk, nome, liquido, preferencia, valor, acrescimo, valor_acrescimo) VALUES ('default', '$id_venda_uni', '$dia', '$hora', '$id_fk','$nome', '$liquido', '$preferencia', '$valor', '$acrescimo', '$valor_acrescimo') ";
                    $result_Insert = $conexao->query($sql_Insert);

                    $sqlDelete = "DELETE FROM pagar_pedido";
